...
;;;
;;; update: 30 Oct 2020 (_AA_)
;;; dashboard/application to boot to needs to be provided in LAUNCH_DATA by the calling XLaunchNewImage
;;; LAUNCH_DATA.Data _must_ be 127 bytes, zero terminated
...
;-------------------------------------
patched_adr:
	dd	3944,	80027743h+5    ; 99h is put at 80027748h when patch is applied
	dd	4034,	80027763h+5
	dd	4817,	800264E7h+5
	dd	4981,	80029D33h+5
	dd	5003,	8002B4B2h+5
...
; 	copy provided dashboard path
	mov     esi, [LaunchDataPage]
	mov 	esi, [esi]
;	actual LAUNCH_DATA.Data is at offset 1024
	add 	esi, 1024
	test 	esi, esi
	jz 		near fatal
	mov 	edi, DASHTOBOOT        
    mov 	ecx, DWORD DASHLEN     
	rep 	movsb 
...
exit:
;-------------------------------------------------------------------------------
; This routine executes the XBE provided by the calling XLaunchNewImage in LAUNCH_DATA 
;-------------------------------------------------------------------------------
	mov     esi, [LaunchDataPage]
	mov		edi, 0x00001000
	mov     ebx, [esi]
...
fatal:
	; flash red led
	mov  	ecx, 0x000000A0	; fast flash red
